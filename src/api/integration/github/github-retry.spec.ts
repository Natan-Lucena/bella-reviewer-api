...
describe("withGithubRetry", () => {
  ...
  it("returns the result on the first successful attempt, without waiting", async () => {
    ...
    const result = await withGithubRetry(fn);
  ...
  it("retries a transient error with exponential backoff until it succeeds", async () => {
    ...
    const pending = withGithubRetry(fn, { attempts: 3, baseDelayMs: 1000 });
  ...
  it("does not retry a permanent error", async () => {
    ...
    const caught = withGithubRetry(fn, { attempts: 3, baseDelayMs: 1000 }).catch(
      (error: unknown) => error,
    );
    await vi.runAllTimersAsync();
