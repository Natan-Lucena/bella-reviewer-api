...
describe("withOpenAiRetry", () => {
  ...
  it("returns the result on the first successful attempt, without waiting", async () => {
    ...
    const result = await withOpenAiRetry(fn);
  ...
  it("retries a transient error with exponential backoff until it succeeds", async () => {
    ...
    const pending = withOpenAiRetry(fn, { attempts: 3, baseDelayMs: 1000 });
  ...
  it("does not retry a permanent error", async () => {
    ...
    const caught = withOpenAiRetry(fn, { attempts: 3, baseDelayMs: 1000 }).catch(
      (error: unknown) => error,
    );
    await vi.runAllTimersAsync();
