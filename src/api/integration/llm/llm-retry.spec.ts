...
describe("withLlmRetry", () => {
  ...
  it("returns the result on the first successful attempt, without waiting", async () => {
    ...
    const result = await withLlmRetry(fn, "TestProvider", PATTERN);
  ...
  it("retries a transient error with exponential backoff until it succeeds", async () => {
    ...
    const pending = withLlmRetry(fn, "TestProvider", PATTERN, { attempts: 3, baseDelayMs: 1000 });
  ...
  it("does not retry a permanent error", async () => {
    ...
    const caught = withLlmRetry(fn, "TestProvider", PATTERN, {
      attempts: 3,
      baseDelayMs: 1000,
