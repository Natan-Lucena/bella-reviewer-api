...
export class LoginUserController extends BaseController {
  ...
  protected async executeImpl(req: Request, res: Response): Promise<Response | void> {
    ...
    // are different origins on Vercel). Both require HTTPS, which local dev
    // over plain http doesn't have, so relax to Lax/non-secure outside
    // production; that's still correct for same-origin tools (curl, Postman,
    // tests) either way.
