...
describe("isDuplicatePendingSuggestion", () => {
  it("returns true when file/line/suggestedCode all match an already-pending suggestion", () => {
  ...
  it("returns false when the line differs, even for the same file/suggestedCode", () => {
    ...
    const isDuplicate = isDuplicatePendingSuggestion(
      { file: "src/a.ts", line: 11, kind: "actionable", suggestedCode: "return items[i - 1];" },
      pending,
    );
  ...
  it("returns false when the file differs", () => {
    ...
    const isDuplicate = isDuplicatePendingSuggestion(
      { file: "src/b.ts", line: 10, kind: "actionable", suggestedCode: "return items[i - 1];" },
      pending,
    );
  ...
  it("returns false when the suggestedCode differs", () => {
    ...
    const isDuplicate = isDuplicatePendingSuggestion(
      { file: "src/a.ts", line: 10, kind: "actionable", suggestedCode: "return items[i];" },
      pending,
    );
  ...
  it("never treats an observation as a duplicate, even if it matches file/line of a pending suggestion", () => {
    ...
    const isDuplicate = isDuplicatePendingSuggestion(
      { file: "src/a.ts", line: 10, kind: "observation", suggestedCode: null },
      pending,
    );
  ...
  it("returns false when there are no pending suggestions at all", () => {
    expect(
      isDuplicatePendingSuggestion(
        { file: "src/a.ts", line: 10, kind: "actionable", suggestedCode: "x" },
        [],
      ),
    ).toBe(false);
  });
